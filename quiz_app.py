"""
结构修理 刷题助手
Firebase Auth 登录 + Realtime Database 存储（零服务账号，加载超快）
"""
...
def clear_data(uid):
    token = _get_id_token()
    if token:
        requests.delete(f"{RTDB_URL}/users/{uid}.json?auth={token}", timeout=10)
...
def make_batch(mode):
    ...
    pool = all_qs
    if mode == "single": pool = [q for q in all_qs if q["type"] == "single"]
    elif mode == "multi": pool = [q for q in all_qs if q["type"] == "multi"]
    elif mode == "judge": pool = [q for q in all_qs if q["type"] == "judge"]
    elif mode == "wrong": pool = [q for q in all_qs if q["id"] in st.session_state.incorrect_ids]
    ...
    with st.form("auth_form", clear_on_submit=False):
        ...
        if st.form_submit_button(btn, type="primary", use_container_width=True):
            if not email or not pw:
                st.error("请填写邮箱和密码"); return
            if "@" not in email or "." not in email.split("@")[-1]:
                st.error("请输入有效的邮箱地址"); return

            if is_reg:
                if confirm != pw: st.error("两次密码不一致"); return
                if len(pw) < 6: st.error("密码至少 6 位"); return
                resp = _auth_post("signUp", {"email": email, "password": pw})
                if "idToken" not in resp:
                    msg = resp.get("error", {}).get("message", "")
                    st.error("该邮箱已被注册" if "EMAIL_EXISTS" in msg else f"注册失败: {msg}")
                    return
                _login_success(resp)
                st.rerun()
            else:
                resp = _auth_post("signInWithPassword", {"email": email, "password": pw})
                if "idToken" not in resp:
                    msg = resp.get("error", {}).get("message", "")
                    st.error("邮箱或密码错误" if "EMAIL_NOT_FOUND" in msg or "INVALID_PASSWORD" in msg else f"登录失败: {msg}")
                    return
                _login_success(resp)
                load_data(resp["localId"])
                st.rerun()

def _login_success(resp):
    ...
    if not done:
        with st.form("answer_form", clear_on_submit=True):
            ...
            if st.form_submit_button("✅ 提交答案", type="primary", use_container_width=True):
                if qtype == "multi":
                    if not sel: st.warning("⚠️ 请至少选择一个选项"); return
                else:
                    if sel is None: st.warning("⚠️ 请选择一个选项"); return
                st.session_state.batch_answers[qid] = sel
                ua = sel
                cl = set(q["answer"].split("|"))
                if qtype == "multi":
                    correct = {a.split(".")[0].strip().upper() for a in ua} == cl
                else:
                    correct = (ua.split(".")[0].strip().upper() if ua else "") in cl
                if correct:
        ...
        for opt in shuffled:
            ...
            i_c, i_s = letter in cl, opt in ua if qtype == "multi" else opt == ua
